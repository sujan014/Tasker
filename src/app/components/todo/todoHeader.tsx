...
const TodoHeader: React.FC<todoHeadProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!todo.item || !todo.description) {
      alert('Enter Todo fields');
      return;
    }
    const newTodo = {
    ...
    try {
      ...
      const responseTodo: TodoInfo = response.data.data;
      handleGroupNewTodo(responseTodo);
      setNewTodo(false);
      setTodo((todo) => ({ ...todo, item: '', description: '' }));
    } catch (error) {
      console.log(error);
    }
  };
